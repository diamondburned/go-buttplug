...
func (p *Payload) UnmarshalJSONFrom(decoder *jsontext.Decoder) error {
	...
	for {
		...
		for {
			...
			case MessageTypeSingleMotorVibrateCmd:
				var msg SingleMotorVibrateCmdMessage
				if err := json.UnmarshalDecode(decoder, &msg); err != nil {
					return fmt.Errorf("failed to unmarshal message of type %q: %w", key, err)
				}
				*p = append(*p, &msg)
...
func (s *SingleMotorVibrateCmdMessage) Type() MessageType {
	return MessageTypeSingleMotorVibrateCmd
}

func (s *SingleMotorVibrateCmdMessage) isMessage()       {}
func (s *SingleMotorVibrateCmdMessage) isClientMessage() {}
...
// WithID implements [ClientMessage].
func (s *SingleMotorVibrateCmdMessage) WithID(id ClientID) ClientMessage {
	s.ID = id
	return s
}
...
func (s *SingleMotorVibrateCmdMessage) LogValue() slog.Value {
	type raw SingleMotorVibrateCmdMessage
	return slog.GroupValue(
		slog.String("type", "SingleMotorVibrateCmd"),
		slog.Any("data", (*raw)(s)))
}
...
type DeviceAddedMessage struct {
	...
	// DeviceMessagesV0: A list of the messages a device will accept on this
	// server implementation.
	DeviceMessages DeviceMessagesV0 `json:"DeviceMessages"`
}
...
// SingleMotorVibrateCmdMessage: Sends a vibrate command to a device that
// supports vibration.
...
// DeviceMessagesV0: A list of the messages a device will accept on this server
// implementation.
type DeviceMessagesV0 []string
